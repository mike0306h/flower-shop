...
def notify_admins(db, channels: list, subject: str, html_content: str, line_message: str = None) -> dict:
    """
    向所有启用的管理员渠道发送通知
    返回发送结果统计
    """
    results = {"email": {"success": 0, "failed": 0}, "line": {"success": 0, "failed": 0}}

    for ch in channels:
        if ch.type == "email":
            ok = send_email(ch.value, subject, html_content)
            if ok:
            ...
            ok = send_line_notify(ch.value, line_message or subject)
            if ok:
                results["line"]["success"] += 1
            else:
                results["line"]["failed"] += 1
...
def build_order_html(order_data: dict, lang: str = "zh") -> tuple:
    """构建订单通知的 HTML 和 LINE 消息"""
    status_map = {
        "zh": {
            "pending": "📋 待确认",
            "confirmed": "✅ 已确认",
...
def send_order_notification(db, order_data: dict, lang: str = "zh") -> dict:
    ...
    html, line = build_order_html(order_data, lang)
    results = notify_admins(db, channels, f"🌸 新订单 - {order_data.get('order_no', '')}", html, line)
    return {"sent": True, **results}


# ============ 预约通知 ============

def build_appointment_html(data: dict, lang: str = "zh") -> tuple:
    """构建预约通知的 HTML 和 LINE 消息"""
    occasion_map = {
...
🎉 场合: {occasion_text}
💰 预算: {budget}
📅 配送: {delivery_date} {delivery_time}
━━━━━━━━━━━━━━━━━━
请及时处理！"""
...
def send_appointment_notification(db, appointment_data: dict, lang: str = "zh") -> dict:
    ...
    html, line = build_appointment_html(appointment_data, lang)
    results = notify_admins(db, channels, f"📅 新预约 - {appointment_data.get('appointment_no', '')}", html, line)
    return {"sent": True, **results}


# ============ 联系我们通知 ============

def build_contact_html(data: dict, lang: str = "zh") -> tuple:
    """构建联系我们通知的 HTML 和 LINE 消息"""
    name = data.get("name", "—")
...
👤 姓名: {name}
📞 电话: {phone}
💬 留言: {message[:50]}{'...' if len(message) > 50 else ''}
━━━━━━━━━━━━━━━━━━
请及时回复！"""
...
def send_contact_notification(db, contact_data: dict, lang: str = "zh") -> dict:
    ...
    html, line = build_contact_html(contact_data, lang)
    results = notify_admins(db, channels, f"📞 联系我们 - {contact_data.get('name', '')}", html, line)
    return {"sent": True, **results}
